...
int main(){
	...
	//Each message is 3 bits, so messages = 0101 0110 as binary where m1 = 101 and m2 = 110
	unsigned char m1 = 0x05, m2 = 0x06;
	...
	unsigned char *c2  = malloc(crypto_kem_CIPHERTEXTBYTES);	
	
	syndrome(c1, pk, e1);
	syndrome(c2, pk, e2);

	fprintBstr(stdout, "c1: ", c1, crypto_kem_CIPHERTEXTBYTES);
	fprintBstr(stdout, "c2: ", c2, crypto_kem_CIPHERTEXTBYTES);	

	unsigned char t1_add[SYS_N/8] = {0};
        unsigned char *t1 = t1_add;

        unsigned char t2_add[SYS_N/8] = {0};
        unsigned char *t2 = t2_add;

	decrypt(t1, sk, c1);
	decrypt(t2, sk, t2);

	fprintBstr(stdout, "t1: ", t1, SYS_N/8);
        fprintBstr(stdout, "t2: ", t2, SYS_N/8);



	//Perform one time pad operation on ciphertexts
	unsigned char *c = malloc(crypto_kem_CIPHERTEXTBYTES);
	otp(c, crypto_kem_CIPHERTEXTBYTES, c1, c2);
	...
	//Decrypt c to find e
	unsigned char e_add[SYS_N/8] = {0};
	unsigned char *e = e_add;
	...
	//TEST *************
	unsigned char exp_add[SYS_N/8] = {0};
	unsigned char *exp = exp_add;
	otp(exp, SYS_N/8, e1, e2);
	fprintBstr(stdout, "exp: ", exp, SYS_N/8);
